...
import { canSendEmail, initEmailJs, sendTrackingEmail } from "./email.js?v=20260520-final";
...
async function handleFormSubmit() {
  ...
  const honeypotValue = document.querySelector('#contact-form [name="website"]')?.value?.trim();
  if (honeypotValue) {
    console.warn("[form] Honeypot ausgelöst. Anfrage verworfen.");
  ...
  try {
    if (canSendEmail()) {
      ...
      console.warn("[EmailJS] SDK nicht verfügbar. PDF wurde trotzdem erstellt.");
    }
  } catch (error) {
    console.warn("[EmailJS] Mailversand fehlgeschlagen. PDF wurde trotzdem erstellt:", error);
  } finally {
